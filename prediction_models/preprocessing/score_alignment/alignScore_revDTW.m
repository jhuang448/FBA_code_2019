function [midi_mat_aligned, notes_altered, dtw_cost, path, jump] = alignScore_revDTW( midi_path, f0_wav, wav, fs_w, wSize, hop )
...
tmp = diff([0 tmp]);
%map = ones(size(wav_pitch_contour_in_midi));
%map(zeros_other) = 0;
noteafterpause(zeros_other) = [];
idx = find(noteafterpause>0);
idx = idx(tmp>15);
...
midi_mat = midi_mat([1; path(pos+1, 1)], :); %update midi_mat, add new notes (due to jumps)
midi_mat_aligned = midi_mat;
midi_mat_aligned(2:end,6) = time_pos; % replace time in midi with time in recording

function rbf(c, g, idx)
display('c ' + string(c));
display('g ' + string(g));
display('idx ' + string(idx));
%clear all;
%close all;
%clc;
warning off;
...
write_file_name = 'featuresFullset135_0327_sr';
savefile = 'feature_selection/rbf_param/' + string(c) + '_' + string(g) + '_' + string(idx) +'.png';
...
%                     114,115,116,117, ...
%                     133,134]+1);
%features = features(:, [1:4 18:21 35:38 52:55 69:end]);
%{
...
for l_id = 1:3 % 3: rhythmic accuracy
    display(l_id)
    labels_i = labels(:,l_id); %labels(:,2),labels(:,3)

    % Evaluate model using cross validation.
    [Rsq_allFeat, S_allFeat, p_allFeat, r_allFeat] = crossValidation_rbf(labels_i, features, NUM_FOLDS, c, g);
    ...
    %save('feature_selection/' + string(l_id) + 'forward_fullset0327_tech','NewList', 'AccuList');
    hold on;
end
hold off;
legend('musicality', 'note acc.', 'rhythmic acc.');
%saveas(gcf, savefile);
close gcf;
%{

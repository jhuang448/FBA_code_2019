...
retV = [mean(v_qnote) std(v_qnote) mean(v_hnote) std(v_hnote) mean(v_wnote) std(v_wnote) noteRatio_old(path, onsetMidi)];
end
...
function [v_note, infn] = tempoPerBeats(v, beatNum, iftempo)
...
for i = 1:l
    if iftempo == 1
        ...
        v_note(i) = sum(v((i-1)*beatNum+1:i*beatNum))/beatNum;
    end
end
